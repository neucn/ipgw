...
func proxyWithUP(c *ctx.Ctx, url, method, headers, body string) {
	// 先登录
	global.LoginWithUP(c, isWebvpn(url))
...
func proxyWithC(c *ctx.Ctx, url, method, headers, body string) {
	// 先登录
	global.LoginWithC(c, isWebvpn(url))
...
func proxy(c *ctx.Ctx, url, method, headers, body string) {
	// 登陆成功，构造请求
	req := buildRequest(c, url, method, headers, body)
...
// 构造请求
func buildRequest(c *ctx.Ctx, url string, method string, headers string, body string) *http.Request {
	// 反序列化headers
	tmp := &http.Header{}
